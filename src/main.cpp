...
static int state = 1; // режим просмотра

GLfloat deltaTime = 0.0f;
GLfloat lastFrame = 0.0f;
GLfloat drawSeaTime = 0.5f; // скорость отрисовки - увеличить для увеличения производительности
...
static int createTriStrip(int rows, int cols, float size, GLuint &vao)
{
	...
	for (uint x = 0; x < terrain_size; ++x)
		...
		{
			max = max < Landscape[x][y] ? Landscape[x][y] : max;
			min = min < Landscape[x][y] ? min : Landscape[x][y];
		}
	for (uint x = 0; x < terrain_size; ++x)
		for (uint y = 0; y < terrain_size; ++y)
		{
	...
	for (int z = 0; z < rows; ++z)
	{
		for (int x = 0; x < cols; ++x)
		{
			...
			float xx = x;
			float zz = z;
			float yy = max_height * Landscape[z][x];
	...
	GL_CHECK_ERRORS;
	{
		...
		glBufferData(GL_ARRAY_BUFFER, texcoords_vec.size() * sizeof(GL_FLOAT),
					 &texcoords_vec[0], GL_STATIC_DRAW);
		GL_CHECK_ERRORS;
...
int initGL()
{
	...
	std::cout << "press left mose button to capture/release mouse cursor  "
			  << std::endl;
	std::cout << "press spacebar to alternate between shaded wireframe and fill "
				 "display modes"
			  << std::endl;
...
int main(int argc, char **argv)
{
	...
	GLFWwindow *window =
		glfwCreateWindow(WIDTH, HEIGHT, "OpenGL basic sample", nullptr, nullptr);
	if (window == nullptr)
	{
	...
	//регистрируем коллбеки для обработки сообщений от пользователя - клавиатура,
	//мышь..
	...
	ShaderProgram oceanShader(shaders);
	GL_CHECK_ERRORS;
	...
	GLuint mountain_texture, sand_texture, water_texture, forest_texture, grass_texture;
	/**** скалы ****/
	// создаем идендификаторы
	glGenTextures(1, &mountain_texture);
	...
	int width, height;
	unsigned char *image = SOIL_load_image("../textures/mountain.png", &width, &height, 0, SOIL_LOAD_RGB);
	...
	image = SOIL_load_image("../textures/sand.png", &width, &height, 0, SOIL_LOAD_RGB);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	glGenerateMipmap(GL_TEXTURE_2D);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	image = SOIL_load_image("../textures/water.png", &width, &height, 0, SOIL_LOAD_RGB);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	glGenerateMipmap(GL_TEXTURE_2D);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	image = SOIL_load_image("../textures/forest.png", &width, &height, 0, SOIL_LOAD_RGB);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	glGenerateMipmap(GL_TEXTURE_2D);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float4x4 projection = projectionMatrixTransposed(
			camera.zoom, float(WIDTH) / float(HEIGHT), 0.1f, 1000.0f);
		...
		//загружаем uniform-переменные в шейдерную программу (одинаковые для всех
		//параллельно запускаемых копий шейдера)
		...
		glBindTexture(GL_TEXTURE_2D, mountain_texture);
		program.SetUniform("mountain_texture", 0);
